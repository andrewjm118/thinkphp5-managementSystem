...
class Pay extends Controller
{
    ...
    public function paylist(){
        ...
        return view('paylist',['result'=>$paylist['data'],'total'=>$paylist['total'],'per_page'=>$paylist['per_page'],'current_page'=>$paylist['current_page'],'search'=>!empty($params['search'])?$v:'','url_params'=>$url_params]);
    }
    //添加交费
    public function payadd(){
        if (request()->isPost()) {
            $params = input('post.');
            $params['payyu'] = $params['payamount'];
            if (!empty($params['paystu'])) {
                $tarr = explode('|',$params['paystu']);
                $params['paystu'] = $tarr[0];
                $params['stuid'] = $tarr[1];
            } else {
                $params['paystu'] = '';
                $params['stuid'] = '';
            ...
            $shou = $this->typeval->where('ti_id',8)->select();
            return view('payadd',['stulist'=>$stulist,'shou'=>$shou]);    
        }
    }
    //编辑课程
    public function payedit(){
        if (request()->isPost()) {
            ...
            $stulist = $this->stu->select();
            $shou = $this->typeval->where('ti_id',8)->select();
            $paydetail = $this->model->where('payid',input('payid'))->find();
            return view('payedit',['paydetail'=>$paydetail,'stulist'=>$stulist,'shou'=>$shou]);
        }
    }
    //删除课程
    public function paydel(){
        $result = $this->model->destroy(input('post.payid'));
        if($result){
